<template>
  <div class="min-h-screen bg-gradient-to-br from-blue-500 to-purple-600 flex flex-col items-center justify-center px-6 py-8">
    ...
    <div class="relative mb-8">
      ...
      <div class="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div 
          v-for="i in 15" 
          :key="`star-${i}`"
          class="absolute w-3 h-3 bg-white rounded-full animate-pulse"
          :style="{
            left: `${Math.random() * 100}%`,
            top: `${Math.random() * 100}%`,
      ...
      <!-- Trophy Icon -->
      <div class="relative z-10 w-32 h-32 bg-yellow-400 rounded-full flex items-center justify-center shadow-2xl animate-bounce">
    ...
    <!-- Workout Stats -->
    <div class="bg-white bg-opacity-20 backdrop-blur-sm rounded-3xl p-6 mb-8 w-full max-w-md">
    ...
    <div class="bg-white bg-opacity-20 backdrop-blur-sm rounded-3xl p-6 mb-8 w-full max-w-md">
      ...
      <div class="space-y-4">
        ...
        <div class="w-full bg-blue-300 bg-opacity-30 rounded-full h-3">
          <div 
    ...
    <div class="w-full max-w-md space-y-4">
      <button 
        ...
        class="w-full bg-yellow-400 text-yellow-900 py-4 rounded-2xl font-bold text-lg shadow-lg hover:bg-yellow-300 transition-colors"
      >
      ...
      <button 
        @click="goToDashboard"
        class="w-full bg-white bg-opacity-20 text-white py-4 rounded-2xl font-bold text-lg backdrop-blur-sm hover:bg-opacity-30 transition-colors"
      >
        Back to Dashboard
      </button>
      ...
      <button 
        @click="shareWorkout"
        class="w-full bg-white bg-opacity-10 text-white py-3 rounded-2xl font-medium backdrop-blur-sm hover:bg-opacity-20 transition-colors flex items-center justify-center space-x-2"
      >
    ...
    <div class="mt-8 text-center">
      <p class="text-white italic text-sm drop-shadow-sm">
        "The only bad workout is the one that didn't happen."
      </p>
      <p class="text-white text-xs mt-1 drop-shadow-sm opacity-90">- Unknown</p>
    </div>
  </div>
</template>
